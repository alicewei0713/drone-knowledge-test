# Drone Knowledge Test 🚁 (AI 智能評測系統)

這是一個使用 React 與 Vite 打造的現代科技風 (Tech Fashion) 無人機知識測驗系統。它透過 Google Apps Script 將 Google 試算表變成輕量級的「雲端資料庫」，能自動讀取題庫與即時寫入玩家的作答成績。

👉 **[Live Demo 線上體驗點這裡](https://alicewei0713.github.io/drone-knowledge-test/)**

## ✨ 系統亮點功能

*   **現代化科技介面**：採用水晶毛玻璃 (Glassmorphism) 設計、輕量化白底藍紫漸層配色，以及流暢的卡片滑動過場動畫。
*   **⏳ 倒數計時挑戰**：每題配有 15 秒倒數進度條，超時將會自動送出並視為錯誤，增加測驗緊張感。
*   **🔙 智能返回與高亮**：允許玩家點擊「上一題」修改答案，並且會以藍色醒目提示玩家原先所選的項目。
*   **🤖 動態 AI 關主**：串接自動生成的精緻 AI 虛擬助手，讓畫面更生動。
*   **純粹雲端維護**：完全不用動程式碼，只要到 Google 試算表打字就能一秒更新題庫。
...
4. 啟動測試伺服器：
   ...
   npm run dev
   ```
   *啟動後在瀏覽器開啟 `http://localhost:5173/` 即可預覽系統畫面。*
...
6. 完成部署與授權後，複製系統配發的 **網頁應用程式網址 (Web app URL)**。
...
1. 在 GitHub 建立一個空的公開 Repository（**不要**勾選 Add a README）。
2. 在本地專案終端機執行以下指令（將你的專案網址替換進去）：
   ```bash
   git init
   git add .
...
3. 上傳成功後，輸入我們設定好的佈署指令：
   ...
   npm run deploy
   ```
4. 看到 `Published` 成功訊息後，等待 1~2 分鐘，前往 GitHub 專案頁面的 **Settings > Pages** 就能看到專屬你的公開遊戲網址了！

---

## 🛠 第四步：後續維護與修改重點

當系統上線後，若要對測試進行修改，請根據你「**想改的地方**」採取不同的處理方式：

### 情況 A：我要修改「題目內容」或「新增題目庫」
最簡單，**你完全不需要修改任何程式碼，也不用重新上傳**！
1. 打開你的 **Google 試算表** 進入「題庫」工作表。
2. 直接刪改、修改或新增題目列。
3. 使用者只要「重新整理網頁」開始新測驗，就會自動抓到你剛儲存的最新題目。

### 情況 B：我要修改「每次抽幾題」或「及格門檻」
遊戲規則的數字設定寫在本地的環境變數檔中，因此需要重新打包發佈。
1. 在本地端電腦打開專案的 **`.env`** 檔案。
2. 更改裡面的這兩項數字：
   - `VITE_QUESTION_COUNT`：每次從大題庫中隨機抽出幾題？ 
   - `VITE_PASS_THRESHOLD`：玩家需要答對幾題才算及格？ 
3. 存檔後，打開終端機執行 `npm run deploy`。
4. 等待 1 分鐘，線上網頁就會自動套用新規則。

### 情況 C：我要修改「UI 顏色/畫面」或「加入新功能」
若你更改了 `.jsx` (畫面/邏輯) 或 `.css` (樣式) 等原始碼範圍：
1. 確保程式碼已經存檔。
2. 在終端機依序執行：
   ```bash
   # 上傳程式碼變更記錄備份到 GitHub
   git add .
   git commit -m "更新您的備註與說明"
   git push

   # 將更新打包並發佈到 Github 靜態網頁上
   npm run deploy
   ```
