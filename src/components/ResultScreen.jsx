    ...
    <div>
      ...
      <div className={`cyber-box ${result.isPassed ? 'success-effect' : 'fail-effect'}`}>
        ...
        {result.details && result.questions && (
          <div className="review-box" style={{ marginTop: '20px', textAlign: 'left', fontSize: '1.1rem', padding: '20px', maxHeight: '350px', overflowY: 'auto' }}>
            <h3 style={{ marginBottom: '15px', color: 'var(--color-secondary)', textAlign: 'center' }}>📝 結算明細</h3>
            <ul style={{ listStyle: 'none', padding: 0 }}>
              {result.details.map((detail, idx) => {
                const qObj = result.questions.find(q => q.originalIndex === detail.originalIndex);
                if (!qObj) return null;
                return (
                  <li key={idx} style={{ marginBottom: '15px', borderBottom: '2px dashed #cbd5e1', paddingBottom: '10px' }}>
                    <div style={{ marginBottom: '8px', color: 'var(--color-text)', fontWeight: '500' }}>
                      <strong>Q{idx + 1}:</strong> {qObj.question}
                    </div>
                    <div style={{ color: detail.isCorrect ? 'var(--color-success)' : 'var(--color-danger)', fontWeight: 'bold' }}>
